...
| `--japan` | `-j` | | 日本全体（与那国島〜択捉島）を固定範囲で描画します | `false` |

## ファイル読み込みの進捗
実行時、指定されたディレクトリ内の対象ファイル数をカウントし、「`ファイルの読み込み完了数/全体数: X/Y`」の形式でコンソールへ進捗状況を表示します。

## 注意事項
* 大量のファイル（数千件）を指定した場合、OpenStreetMap のタイルサーバーへリクエストが多数発生し、タイル取得および生成処理に時間がかかる場合があります。
