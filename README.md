...
* **任意の領域指定**: `--bounds "minLon,minLat,maxLon,maxLat"` オプションで、任意の領域を指定して描画することができます。
...
# 指定した領域（例: 東京周辺）のみを描画
node index.js --input ./my_routes --output ./my_map.png --bounds "139.5,35.5,140.0,35.8"
```
...
| `--bounds <string>`| | | 指定した領域(minLon,minLat,maxLon,maxLat)のみを固定範囲で描画します | |
